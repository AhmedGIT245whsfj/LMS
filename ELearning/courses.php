...
if ($trackId > 0) {
  ...
  if ($stmtT) {
    ...
    $selectedTrack = ($resT && $resT->num_rows === 1) ? $resT->fetch_assoc() : null;
    $stmtT->close();
  }
...
<div class="container mt-5">
  ...
  <?php if ($trackId > 0 && $selectedTrack): ?>
    <h1 class="text-center"><?php echo htmlspecialchars($selectedTrack['track_name']); ?></h1>
    <p class="text-center text-muted mb-4"><?php echo htmlspecialchars($selectedTrack['track_desc']); ?></p>
          ...
          <div class="col-sm-6 col-lg-3 mb-4">
            <a href="courses.php?track_id=<?php echo (int)$t['track_id']; ?>" class="btn" style="text-align:left; padding:0px; width:100%;">
              <div class="card h-100">
                <?php
                $timg = '';
                if (isset($t['track_img'])) {
                  $timg = trim((string)$t['track_img']);
                }
                ?>
                <?php if ($timg !== ''): ?>
